package com.sushkoolesua.kotlincourse.lesson6

fun main () {
    println(seasonOfYear(2))
    println(petAge(3))
    println(modeOfTransport(1.0))
    println(bonusPoint(200))
    println(documentsType("txt"))
    println(temperatureConverter) //вот тут вот вообще не поняла как
    println(typeOfClothes(-10))
    println(filmsCategory(12))
}
...
fun seasonOfYear(month:Int):String {
...
fun modeOfTransport(distance:Double): String {
...
// покупки и возвращает количество бонусных баллов: 2 балла за каждые 100 рублей при сумме покупки до 1000 рублей и
// 5 баллов за каждые 100 рублей при сумме свыше этого.

fun bonusPoint(orderAmount: Int) : Int {
    return if (orderAmount <= 1000) {
        (orderAmount/100) * 2
    } else orderAmount/100 * 5
}
...
fun documentsType(fileExtension: String): String {
...
//Подсказка: для генерации строки из числа и буквы можно использовать шалон “$result F” для фаренгейта или
// “$result C” для цельсия

fun temperatureConverter (temperatures: Int, typeTemperature: String) : String {
    return if (typeTemperature == "F") {
        $result F ? // не понятно
    } else
}
...
fun typeOfClothes(temperature: Int): String {
...
fun filmsCategory (age: Int) : String {
    return when (age) {
        in 0..10 -> "Детские"
        in 11..17 -> "Подростковые"
        else -> "18+"
    }
}